...
namespace Fte.Ioc.Tests.Resolver
{
	...
	public class ObjectFactoryTest
	{
		...
		{
			_factory.Create(_singletonRegItem, new object[0]);
			var result = _factory.HasInstance(_singletonRegItem);
		...
		{
			_factory.Create(_transientRegItem, new object[0]);
			var result = _factory.HasInstance(_transientRegItem);
		...
		public void GetInstance_SingletonHasBeenCreated_AlwaysReturnsSameObject()
		{
			...
			var obj1 = _factory.GetInstance(_singletonRegItem);
			var obj2 = _factory.GetInstance(_singletonRegItem);
		...
		{
			_factory.GetInstance(_singletonRegItem);
		}
	}
}
